...
public class MyApprDoneView  extends Composite{
	
	public MyApprDoneView(Composite com){
		super(com,SWT.NONE);
	   	 content=this;
	   	 this.createAndShow();
	}
	...
	public void createAndShow(){
		...
		@Override
		   ...
		   List<DelayDate> data=DELAYDETAIL.getApprovedList("", Context.session.userID, DelayDate.Status.Apply.ordinal()+"", "");
		   if(data!=null&&data.size()>0){
			   ...
			   for(DelayDate apply:data){
				   ...
				   tableItem.setText(new String[]{apply.getApplyID(),apply.getApplyUserFullName(),apply.getApplyTime(),apply.getName(),apply.getReason(),statusName,apply.getComment(),apply.getMtime()});
	...
	public class SearchAction extends SelectionAdapter{
		public void widgetSelected(SelectionEvent e){
			...
			dataTable.removeAll();
			 List<DelayDate> data=DELAYDETAIL.getApprovedList(nextUserID, Context.session.userID, DelayDate.Status.Apply.ordinal()+"", key);
			  if(data!=null&&data.size()>0){
				   Map<String,String> Status=Dictionary.getDictionaryMap("DELAY_DETAIL.DSTATUS");
				   for(DelayDate apply:data){
					   TableItem tableItem=new TableItem(dataTable,SWT.BORDER);
