...
public class AuditorEditView extends EditView{
	private static AuditorEditView unique_instance=null;
	public static  AuditorEditView getInstance(Composite parent){
		if (unique_instance==null){
			unique_instance=new AuditorEditView(parent,SWT.NONE);
		}
	...
	private AuditorEditView(Composite com,int style){
				...
				if(itemName.equals(items[k].getText())){
		            ...
		            if(tabCount==0) {
		            	WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
						  AuditorEditView.getInstance(null).setFolderMaxed(false);
		...
		public class CTabFullScreenAction extends MouseAdapter{
			public void mouseDoubleClick(MouseEvent e){
			  if(!isFolderMaxed){
				  ...
				  AuditorEditView.getInstance(null).setFolderMaxed(true);
				  WorkAreaView.getInstance(null).getSachForm().layout(true);
			  }else{
				  WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
				  AuditorEditView.getInstance(null).setFolderMaxed(false);
