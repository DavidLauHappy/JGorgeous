...
public class DManagerEditView extends EditView{
	private static DManagerEditView unique_instance=null;
	public static  DManagerEditView getInstance(Composite parent){
		if (unique_instance==null){
			unique_instance=new DManagerEditView(parent,SWT.NONE);
		}
	...
	private DManagerEditView(Composite com,int style){
				...
				if(itemName.equals(items[k].getText())){
		            ...
		            if(tabCount==0) {
		            	WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
						  DManagerEditView.getInstance(null).setFolderMaxed(false);
		...
		public class CTabFullScreenAction extends MouseAdapter{
			public void mouseDoubleClick(MouseEvent e){
			  if(!isFolderMaxed){
				  ...
				  DManagerEditView.getInstance(null).setFolderMaxed(true);
				  WorkAreaView.getInstance(null).getSachForm().layout(true);
			  }else{
				  WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
				  DManagerEditView.getInstance(null).setFolderMaxed(false);
