...
public class AdminEditView extends EditView{
	private static AdminEditView unique_instance=null;
	public static  AdminEditView getInstance(Composite parent){
		if (unique_instance==null){
			unique_instance=new AdminEditView(parent,SWT.NONE);
		}
	...
	private AdminEditView(Composite com,int style){
	...
	public void showEditView(){
		tabFloder=new CTabFolder(comSelf,SWT.TOP|SWT.CLOSE);//|SWT.CLOSE|
		...
		//拓扑部署tab页是默认的
	/*	 CTabItem itemTopologyAsDflt=new CTabItem(tabFloder,SWT.NONE);
		 itemTopologyAsDflt.setText(Constants.getStringVaule("tabItem_verdeploy"));
		 deployView=new DeployView(tabFloder);
		
		 itemTopologyAsDflt.setControl(deployView.getSachForm());
		 tabFloder.setSelection(itemTopologyAsDflt);*/
		 tabFloder.pack();
				...
				if(itemName.equals(items[k].getText())){
		            ...
		            if(tabCount==0) {
		            	WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
						  AdminEditView.getInstance(null).setFolderMaxed(false);
		...
		public class CTabFullScreenAction extends MouseAdapter{
			public void mouseDoubleClick(MouseEvent e){
			  if(!isFolderMaxed){
				  ...
				  AdminEditView.getInstance(null).setFolderMaxed(true);
				  WorkAreaView.getInstance(null).getSachForm().layout(true);
			  }else{
				  WorkAreaView.getInstance(null).getSachForm().setMaximizedControl(null);
				  AdminEditView.getInstance(null).setFolderMaxed(false);
