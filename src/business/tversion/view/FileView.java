...
public class FileView extends Composite{
    ...
    public static FileView getInstance(Composite parent){
   	 if(unique_instance==null)
   		 if(parent!=null){
   			 unique_instance=new FileView(parent);
   		 }
   	 return unique_instance;
    } 
    
    private FileView(Composite parent){
  		...
  		dbNodeSource.addDragListener(new DragSourceListener() {
			...
			public void dragSetData(DragSourceEvent event) {
				if(FileTransfer.getInstance().isSupportedType(event.dataType)){
					TreeItem[] dragItems=fileTree.getSelection();
					String[] items=new String[dragItems.length];
					for(int w=0;w<dragItems.length;w++){
						String filePath=((File)dragItems[w].getData()).getAbsolutePath();
						items[w]=filePath;
					}
					event.data=items;
				}
   ...
   public static boolean startFullWith(String longer,String shorter){
    ...
    private static FileView unique_instance;
