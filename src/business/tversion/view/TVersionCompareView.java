...
public class TVersionCompareView {
	...
	public TVersionCompareView(Composite com){
		this.parent=com;
				  ...
				  if(tgtFile==null){
					  ...
					  if(downFromRet&&downToRet){
						  String lab=file.getFileName()+":"+file.getVersionID()+" vs "+tgtFile.getVersionID();
							if(!TVersionEditView.getInstance(null).getTabState(lab)){
								TVersionTextCmpView compareView=new TVersionTextCmpView(TVersionEditView.getInstance(null).getTabFloder(),file,tgtFile);
								TVersionEditView.getInstance(null).setTabItems(compareView.self, lab);
				   			}
					  }else{
						  String msg="文件【"+file.getFileName()+"】下载失败。请稍后重试！";
							MessageBox box=new MessageBox(AppView.getInstance().getShell(),SWT.ICON_INFORMATION|SWT.OK);
