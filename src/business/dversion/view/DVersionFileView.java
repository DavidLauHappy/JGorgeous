...
public class DVersionFileView  extends Composite{
	public static DVersionFileView getInstance(Composite parent){
	   	 if(unique_instance==null)
	   		 if(parent!=null){
	   			 unique_instance=new DVersionFileView(parent);
	   		 }
	   	 return unique_instance;
	    } 
	    
	    private DVersionFileView(Composite parent){
	    ...
	    private void createSearchPanel(){
	    	...
	    	Button btnSearch=new Button(panel,SWT.PUSH);
	    	btnSearch.setLayoutData(LayoutUtils.getComGridData(SWT.CENTER, SWT.CENTER, false, false, 1, 1, 0, 0));
	    	btnSearch.setImage(Icons.getSearchIcon());
	    	btnSearch.addSelectionListener(new FindByPathAction());
	    	btnSearch.setToolTipText(Constants.getStringVaule("btn_tips_findByPath"));
	    	panel.pack();
	    }
	  		...
	  		dbNodeSource.addDragListener(new DragSourceListener() {
				...
				public void dragSetData(DragSourceEvent event) {
					if(FileTransfer.getInstance().isSupportedType(event.dataType)){
						...
						//TreeItem dragItem=fileTree.getSelection()[0];
						//String filePath=((File)dragItem.getData()).getAbsolutePath();
	    ...
	    private static DVersionFileView unique_instance;
