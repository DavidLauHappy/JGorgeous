...
public class DVFileSearchView {
	private static DVFileSearchView unique_instance;
	private  Shell Result=null;
	private org.eclipse.swt.widgets.List listControl=null;
		public static DVFileSearchView getSearchView(){
			if(unique_instance==null)
				unique_instance=new DVFileSearchView();
			return unique_instance;
		}
		
		private DVFileSearchView(){
			...
			public void mouseDoubleClick(MouseEvent e) {
				...
				if(listControl.getSelectionCount()>0){
					...
					if(columns!=null&&columns.length==2){
				    	...
				    	//DVersionFileView.getInstance(null).setSelection(filePath);
				    	//DVFileSearchView.getSearchView().exit();
					}
				}
			}
