...
public class AllReqSearchView {
	...
	public AllReqSearchView(Composite com){
		this.parent=com;
			...
			List<TaskBean> reqs=TASK.getAllTopTasks(currentNum+"",currentNum+PageCount+"");
			 if(reqs!=null&&reqs.size()>0){
				 for(TaskBean req:reqs){
					 ...
					 String appName=Apps.get(req.getApp());
					 String step=Process.get(req.getStatus());
					 String currUser=req.getCurrentUser()+"("+req.getCurrentUser()+")";
	     	          tableItem.setText(new String[]{req.getCrtTime(),req.getReqID(),req.getOwnerShowName(),req.getTname(),req.getId(),step, appName,currUser}); 
	     	          tableItem.setData(req);
				 }
			 }
	 		...
	 		public void mouseDown(MouseEvent e){}
	 		 public void mouseDoubleClick(MouseEvent e){
	 			 ...
	 			 if(currentItem!=null){
	 				...
	 				if(!DVersionEditView.getInstance(null).getTabState(tab)){
	 					ReqDetailView rdv=new ReqDetailView(DVersionEditView.getInstance(null).getTabFloder(),bean);
	 					DVersionEditView.getInstance(null).setTabItems(rdv.content, tab);
	 				}
	 			 }
	 		 }
	  }
