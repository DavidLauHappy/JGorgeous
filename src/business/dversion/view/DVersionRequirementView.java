...
public class DVersionRequirementView extends Composite{
	private static DVersionRequirementView unique_instance;
	
	  private Composite content=null;
	  private Tree tree;
	  public static DVersionRequirementView getInstance(Composite com){
		  if(unique_instance==null)
			  unique_instance=new DVersionRequirementView(com);
		  return unique_instance;
	  }
	  
	  private DVersionRequirementView (Composite parent){
	 		 ...
	 		 public void mouseDoubleClick(MouseEvent e){
	 			...
	 			if(currentItem!=null){
	 				...
	 				if(!DVersionEditView.getInstance(null).getTabState(item)){
	 					ReqDetailView rdv=new ReqDetailView(DVersionEditView.getInstance(null).getTabFloder(),req);
	 					DVersionEditView.getInstance(null).setTabItems(rdv.content, item);
	 				}
	 			}
	 		 }
	  }
	  ...
	  public class FliterReqAction  extends SelectionAdapter{
			public void widgetSelected(SelectionEvent e){
				...
				AllStatus.add(new Item("TASK_DEF.STATUS","0","È«²¿"));
