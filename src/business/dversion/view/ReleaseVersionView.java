...
import business.dversion.action.DragAction;
 ...
 *
 */
public class ReleaseVersionView {
	...
	public ReleaseVersionView(Composite com,ViewBean viewData){
		this.data=viewData;
	...
	private void createDrawView(){
		...
		for(ViewVersionBean version:versions){
			...
			y=y+100;
			VersionPropertyView propertyView=new VersionPropertyView(version);
			figure.setPropertyView(propertyView);
			new  DragAction(figure,this);
		...
		if(uniqueMenu==null){
						...
						if(currentFigure!=null){
							...
							String itemName=data.getViewName()+"_"+data.getShowVersion();
	        	   			if(!DVersionEditView.getInstance(null).getTabState(itemName)){
	        	   				ViewVersionView versionView=new ViewVersionView(DVersionEditView.getInstance(null).getTabFloder(),data);
	        	   				DVersionEditView.getInstance(null).setTabItems(versionView.self, itemName);
	        	   			}
						}
				 	}
				});
				...
				itemCmp.addSelectionListener(new SelectionAdapter() { 
					public void widgetSelected(SelectionEvent e){
						...
						if(!DVersionEditView.getInstance(null).getTabState(itemName)){
							 DVersionCompareView compareView=new DVersionCompareView(DVersionEditView.getInstance(null).getTabFloder());
							 compareView.setDataLeft(data);
							 DVersionEditView.getInstance(null).setTabItems(compareView.self, itemName);
						}
				 	}
				});
		}
