...
import business.tester.action.TesterDragAction;
...
public class TesterReleaseVersionView {
	...
	public TesterReleaseVersionView(Composite com,ViewBean viewData){
		this.data=viewData;
	...
	private void createDrawView(){
		...
		for(ViewVersionBean version:versions){
			...
			y=y+100;
			TesterPropertyView propertyView=new TesterPropertyView(version);
			figure.setPropertyView(propertyView);
			new  TesterDragAction(figure,this);
		...
		if(uniqueMenu==null){
						...
						if(currentFigure!=null){
							...
							String itemName=data.getViewName()+"_"+data.getShowVersion();
	        	   			if(!TesterEditView.getInstance(null).getTabState(itemName)){
	        	   				TesterViewVersionView versionView=new TesterViewVersionView(TesterEditView.getInstance(null).getTabFloder(),data);
	        	   				TesterEditView.getInstance(null).setTabItems(versionView.self, itemName);
	        	   			}
						}
				 	}
				});
				...
				itemCmp.addSelectionListener(new SelectionAdapter() { 
					public void widgetSelected(SelectionEvent e){
						...
						if(!TesterEditView.getInstance(null).getTabState(itemName)){
							 TesterCompareView compareView=new TesterCompareView(TesterEditView.getInstance(null).getTabFloder());
							 compareView.setDataLeft(data);
							 TesterEditView.getInstance(null).setTabItems(compareView.self, itemName);
						}
				 	}
				});
		}
