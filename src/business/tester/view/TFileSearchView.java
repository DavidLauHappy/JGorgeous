...
public class TFileSearchView {
	private static TFileSearchView unique_instance;
	private  Shell Result=null;
	private org.eclipse.swt.widgets.List listControl=null;
		public static TFileSearchView getSearchView(){
			if(unique_instance==null)
				unique_instance=new TFileSearchView();
			return unique_instance;
		}
		
		private TFileSearchView(){
			...
			public void mouseDoubleClick(MouseEvent e) {
				...
				if(listControl.getSelectionCount()>0){
					...
					if(columns!=null&&columns.length==2){
				    	...
				    	//TesterFileView.getInstance(null).setSelection(filePath);
				    	//TFileSearchView.getSearchView().exit();
					}
				}
			}
